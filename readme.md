...
- **Pairing support** - Register ESP32 as a remote control using `substitutions.pairing_mode`: `standard` sends Join only (3 transmissions), `compat` sends Join2 only (3 transmissions), and the default `both` sends Join followed by Join2 (6 transmissions total). Leave command behavior is unchanged: 30 transmissions with 4ms delay between each, plus transmission overhead - approximately 1 second total per ITHO specification
...
1. **Wait for confirmation:**
  The unit should shortly vary the fan speed to confirm pairing. Check the ESP32 logs for `Pairing mode: ...` first. In `standard` mode, logs will show `Sending Join command (counter=X)` and the Join packet is transmitted 3 times with 40ms delay. In `compat` mode, logs will show `Sending Join2 (compat) command (counter=X)` and the Join2 packet is transmitted 3 times with 40ms delay. In the default `both` mode, logs will show both Join lines, with Join sent 3 times and Join2 sent 3 times, for 6 pairing transmissions total. The device ID is automatically generated from the last 3 bytes of the ESP32 MAC address.
...
- **Join commands**: `standard` mode sends Join 3 times with 40ms delay, `compat` mode sends Join2 3 times with 40ms delay, and the default `both` mode sends Join 3 times followed by Join2 3 times with a 200ms gap between the two sequences
